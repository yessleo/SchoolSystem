...
import { getclase } from 'src/actions/clases';
...
export function StudentTableRow({ row, selected, onEditRow, onSelectRow, onDeleteRow }) {
  ...
  useEffect(() => {
    getclase(row.clase).then(result => setcurrentClase(result.data));
  }, []);
  ...
  return (
    <>
      <TableRow hover selected={selected} aria-checked={selected} tabIndex={-1}>
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{currentClase.nombre}</TableCell>
      ...
      <StudentQuickEditForm currentStudent={row} open={quickEdit.value} onClose={quickEdit.onFalse} />
